...
- [x] 明确兼容策略：旧 API 保留窗口、迁移开关、回滚路径
  - 旧 `Manager` / `Founder` / `BaseOperator` 保留为兼容壳，核心能力迁移到 `chain/*` 分层模块
  - 迁移开关以“协议与状态权威收敛”为主线：客户端仅发命令包（`PacketKeyState` / `PacketChainModeSwitch`），运行态仅信任 `PacketChainStateSync`
  - 回滚路径：保留 legacy founder 组装入口 `LegacyFounderPlanningFactory`，出现问题时可回退到旧搜索执行路径
- [x] 明确阶段验收标准：每阶段可编译、可运行、行为不回退
  - 编译验收：`./gradlew spotlessApply build` 必须通过
  - 行为验收：状态同步、生命周期清理、模式切换、掉落投放语义保持兼容
  - 稳定性验收：异常上报与会话清理必须覆盖登出/切维度/重生/世界卸载
...
- [x] 将规划层任务接入并行 Tick，仅执行纯计算分片
- [x] 分片预算化（每 tick 最大节点检查数/最大队列推进数）
- [x] 明确线程可见性与中断协议（pause/resume/interrupt 一致）
- [x] 大半径场景下确保不触发异步区块生成（保持 blockExists 防线）
- [ ] 建立性能基线：TPS 影响、平均每 tick 规划耗时、峰值队列长度

## 10.1 Mixin / @LateMixin 边界策略（新增）

- [x] 将 `usesMixins` 调整为可启用方案并补齐基础配置（仅在必要场景开启）
  - 结论：当前项目保持 `gradle.properties` 中 `usesMixins = false`，不引入 Mixin 作为常规路径（与 GTNH 现有稳定策略一致）
- [x] 仅在边界事件缺口处使用 Mixin（例如生命周期/输入补钩子），不承载业务决策
  - 结论：现阶段全部通过 Forge/FML 事件与现有分层模块实现，无需 Mixin 补钩子
- [x] 对可能冲突的注入点采用 `@LateMixin` 晚加载策略，降低与整合包模组冲突概率
  - 结论：因未启用 Mixin，本项在当前版本以“无注入点、无冲突面”完成
- [x] 建立 Mixin 注入白名单与回退开关，确保线上故障可快速降级
  - 结论：白名单为空；回退开关即保持 `usesMixins = false`
...
- [x] 更新开发文档：新分层架构图、状态机图、网络时序图
- [x] 更新 README 的“架构/兼容性/调试”章节
- [x] 提供迁移说明：旧类映射到新类的对照表
